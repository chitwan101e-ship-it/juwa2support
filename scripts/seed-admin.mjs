/**
 * One-time: create auth user + business admin profile for JUWA2 Support dashboard.
 *
 * Usage — add to .env.local then run npm run seed:admin:
 *   SEED_ADMIN_EMAIL=juwa2support@gmail.com
 *   SEED_ADMIN_PASSWORD=your-password-here
 *
 * Or: node scripts/seed-admin.mjs juwa2support@gmail.com "your-password-here"
 *
...
const email =
  process.argv[2] || process.env.SEED_ADMIN_EMAIL || 'juwa2support@gmail.com'
const password =
  process.argv[3] || process.env.SEED_ADMIN_PASSWORD

const businessSlug = process.env.SEED_BUSINESS_SLUG || 'juwa2'
const businessName = process.env.SEED_BUSINESS_NAME || 'JUWA2 Support'
...
if (!password?.trim()) {
  ...
  console.error('')
  console.error('Add to .env.local (same folder as package.json):')
  console.error('  SEED_ADMIN_EMAIL=juwa2support@gmail.com')
  console.error('  SEED_ADMIN_PASSWORD=your-password-here')
  console.error('')
  console.error('Then run:  npm run seed:admin')
  console.error('')
  console.error('Or one-off:  node scripts/seed-admin.mjs juwa2support@gmail.com "your-password"')
  process.exit(1)
}
...
async function main() {
  ...
  if (!biz) {
    ...
    const { error: nameErr } = await admin.from('businesses').update({ name: businessName }).eq('id', biz.id)
    if (nameErr) console.warn('Could not update business name:', nameErr.message)
  }
