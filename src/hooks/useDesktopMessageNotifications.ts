      ...
      (payload) => {
        void (async () => {
          ...
          showDesktopNotification({
            title,
            body: row.body,
            tag: `juwa2-notify-${row.id}`,
            onClick: () => {
              // Ticket notifications carry a direct dashboard link. Prefer it over
              // conversation_id so clicking the popup opens the ticket workspace.
              if (row.link?.includes('ticket=')) onOpenRef.current?.(row)
              else if (row.conversation_id) onOpenConvoRef.current?.(row.conversation_id)
