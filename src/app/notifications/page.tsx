...
import { Bell, ArrowLeft, ArrowRight, Loader2, CheckCheck, Home, User } from 'lucide-react'
...
export default function NotificationsPage() {
  ...
  return (
    <div className="min-h-screen bg-[#000000] pb-28 text-white">
      <header className="sticky top-0 z-40 bg-[#0c0c0c]/95 border-b border-white/10 backdrop-blur">
        <div className="max-w-3xl mx-auto px-3 sm:px-4 h-14 flex items-center gap-3">
          ...
          <Bell className="w-5 h-5 text-[#f5d040]" />
          ...
          <div className="ml-auto flex items-center gap-1">
            ...
            <button
              ...
              className="text-sm font-medium text-[#f5d040] disabled:opacity-40 px-2 py-1"
            >
      ...
      <main className="max-w-3xl mx-auto p-4">
        {items.length === 0 ? (
          <div className="bg-[#0c0c0c]/95 rounded-xl border border-white/10 p-10 text-center text-[#8f99b8]">
          ...
          <div className="space-y-3">
            {items.map((n) => (
              <article
                key={n.id}
                className={`bg-[#0c0c0c]/95 rounded-xl border p-4 transition ${
                  n.read ? 'border-white/10' : 'border-[#f5d040]/40 shadow-[0_0_0_1px_rgba(141,99,255,0.1)]'
                }`}
              >
                <div className="flex items-start gap-3">
                  <div
                    className={`w-9 h-9 rounded-full flex items-center justify-center ${
                      n.read ? 'bg-white/5' : 'bg-[#f5d040]/15'
                    }`}
                  >
                    <Bell className={`w-4 h-4 ${n.read ? 'text-[#6f7896]' : 'text-[#f5d040]'}`} />
                  </div>
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      <h2 className="font-semibold text-white truncate">{n.title}</h2>
                      {!n.read ? <span className="w-2 h-2 rounded-full bg-[#f5d040]" /> : null}
                    </div>
                    <p className="text-sm text-[#b8c0dc] mt-1 whitespace-pre-wrap">{n.body}</p>
                    <div className="mt-2 flex flex-wrap items-center justify-between gap-2">
                      <span className="text-xs text-[#7f8bad]">{timeAgo(n.created_at)}</span>
                      {n.link ? (
                        <button
                          type="button"
                          onClick={() => void openNotification(n)}
                          className="inline-flex items-center gap-1 rounded-lg border border-[#f5d040]/25 bg-[#f5d040]/10 px-2.5 py-1 text-xs font-semibold text-[#f5d040] hover:bg-[#f5d040]/15"
                        >
                          Open
                  ...
                  {!n.read ? (
                    <button
                      ...
                      disabled={busy === n.id}
                      className="text-[#7f8bad] hover:text-[#f5d040] p-1 rounded"
      ...
      {portalRole === 'customer' ? (
        ...
        <nav
          className="juwa2-footer-bar fixed bottom-0 left-0 right-0 z-30 flex border-t border-white/[0.08] bg-[#090e20]/96 backdrop-blur-[16px] min-[900px]:hidden"
        ...
        >
          ...
          <Link
            href="/notifications"
            className="relative flex flex-1 flex-col items-center gap-0.5 py-1.5 text-[10px] font-semibold text-[#f5d040]"
          >
