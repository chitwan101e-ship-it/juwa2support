...
const MAX_TECHNICAL_AGENTS = 4

const EMAIL_RE = /^[^\s@]+@[^\s@]+\.[^\s@]+$/

/** Admin-only: create a technical support staff account. */
export async function POST(req: NextRequest) {
  try {
    ...
    const {
      data: { user },
      error: sessionErr,
    } = await withTimeout(supabase.auth.getUser(), 15_000, 'Session check timed out.')
    if (sessionErr) {
    ...
    if (
      ...
      return NextResponse.json({ error: 'Only business admins can add technical staff.' }, { status: 403 })
    }

    const businessId = adminProf.business_id as string
    const admin = createServiceClient()

    const { count: technicalCount, error: cErr } = await withTimeout(
      admin
        .from('profiles')
        .select('*', { count: 'exact', head: true })
        .eq('business_id', businessId)
        .eq('business_role', 'technical')
        .is('deleted_at', null),
      15_000,
      'Technical count check timed out.'
    )
    ...
    if ((technicalCount ?? 0) >= MAX_TECHNICAL_AGENTS) {
      return NextResponse.json(
        { error: `This business already has ${MAX_TECHNICAL_AGENTS} technical agents. Remove someone before adding another.` },
        { status: 400 }
      )
    }
    ...
    const { data: authData, error: authErr } = await withTimeout(
      ...
      45_000,
      'Supabase Auth did not create the user in time.'
    )
    ...
    const { error: insErr } = await withTimeout(
      admin.from('profiles').insert({
        ...
        business_role: 'technical',
        support_inbox_scope: null,
        account_status: 'approved',
        email_verified: true,
      }),
      20_000,
      'Saving the staff profile timed out.'
    )
    ...
    return NextResponse.json({
      ...
      username,
      email,
      message: 'Technical staff created. They sign in at /login and see the Technical Escalations inbox.',
    })
  } catch (e) {
