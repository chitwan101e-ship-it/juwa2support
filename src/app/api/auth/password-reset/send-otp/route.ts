...
export async function POST(req: NextRequest) {
  try {
    ...
    const { error: emailError } = await getResend().emails.send({
      ...
      to: target.authEmail,
      subject: `Your ${JUWA2_BRAND} password reset code`,
      html: `
        <div style="font-family: sans-serif; max-width: 480px; margin: 0 auto; padding: 32px;">
          <h1 style="font-size: 24px; color: #d4af37; margin-bottom: 8px;">${JUWA2_BRAND}</h1>
    ...
    if (lower.includes('auth_user_id_for_email') || lower.includes('schema cache')) {
      return NextResponse.json(
        {
          error:
