...
function applyAppearanceClass(mode: AppearanceMode) {
  ...
  root.classList.remove('juwa2-theme-dark', 'juwa2-theme-light', 'juwa2-theme-playful')
  root.classList.add(`juwa2-theme-${mode}`)
}
...
export default function ProfilePage() {
  ...
  const cardBg = 'bg-[#0c0c0c]/95 border-white/10'
  const mutedText = 'text-[#7f8bad]'
  const titleText = 'text-white font-semibold'
  const bodyText = 'text-[#c4cbe6]'

  const appearanceOptions = [
    { id: 'dark' as const, label: 'Dark', icon: Moon, hint: 'Default theme' },
  ...
  return (
    <div className={`min-h-screen text-[14px] leading-snug ${pageBg}`}>
      <header className="px-4 pt-3 pb-2 flex items-center gap-2 border-b border-white/[0.08] bg-[#0c0c0c]/40 backdrop-blur-sm">
        <button
          type="button"
      ...
      >
        <div className="flex items-start gap-3">
          <div className="relative shrink-0">
            ...
            {profile.avatar_url ? (
              <img
                ...
                className="w-16 h-16 rounded-full object-cover border border-white/10 bg-[#141414]"
              />
            ) : (
          ...
          <div className="min-w-0 flex-1 pt-0.5">
            ...
            <p className={`${mutedText} text-[13px] mt-0.5 truncate`}>{email || `${profile.username}@juwa2.app`}</p>
            ...
            {appearanceOptions.map(({ id, label, icon: Icon, hint }) => (
              <button
                ...
                className={`rounded-xl border px-2 py-2.5 text-center transition-all ${
                  appearance === id
                    ? 'border-[#f5d040] bg-[#1a2040] text-[#f5d040]'
                    : 'border-white/10 bg-[#141414] text-[#7f8bad] hover:border-white/20'
                }`}
              >
