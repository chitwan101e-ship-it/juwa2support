...
export async function resolveCustomerRecipient(
  ...
  if (looksLikeEmail(raw)) {
    ...
    const { data: userId, error: rpcErr } = await admin.rpc('auth_user_id_for_email', {
      p_email: emailLower,
    })
...
/** Profiles for business-linked customers only (chunked, parallel). */
export async function listBusinessMemberProfiles(
  ...
  const chunkResults = await Promise.all(
    chunkIds(memberIds, MEMBER_PROFILE_CHUNK).map(async (slice) => {
      const { data, error } = await client
        .from('profiles')
...
export async function searchCustomersForNotify(
  ...
  const profileChunks = await Promise.all(
    chunkIds(memberIds, idChunk).map(async (slice) => {
  ...
  for (const chunk of profileChunks) profiles.push(...chunk)
  ...
  const emailHitId = looksLikeEmail(q)
    ? ((await admin.rpc('auth_user_id_for_email', { p_email: q })).data as string | null)
    : null
