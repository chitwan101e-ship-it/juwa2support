...
export function PendingApprovalPanel({ embedded = false, needsSignIn = false }: PendingApprovalPanelProps) {
  ...
  async function checkNow() {
    ...
    try {
      ...
      setCheckFeedback('Still waiting for approval. This page checks automatically.')
    } finally {
      setChecking(false)
    }
  }
  ...
  if (needsSignIn) {
    return (
      <div className="text-center py-2 space-y-4">
        <div className="w-14 h-14 rounded-2xl bg-[#2c220f] flex items-center justify-center mx-auto text-xl">
          ✓
        </div>
        <div>
          <h2 className="font-semibold text-lg mb-1.5 text-white">Request submitted</h2>
          <p className="text-[#8b96b8] text-sm leading-relaxed">
            Sign in with the same email and password. We&apos;ll open your feed once staff approves you.
          </p>
        </div>
        <button
          type="button"
          onClick={() => router.push('/login')}
          className="w-full py-3 rounded-xl juwa2-btn font-semibold hover:opacity-95 transition-opacity"
        >
          Sign in
        </button>
      </div>
  ...
  const subtitle =
    status === 'pending' && embedded ? (
      <p className="text-[#f5d040] text-sm font-semibold">Waiting for approval</p>
    ) : null

  const body =
    status === 'rejected'
      ? 'Your signup was not approved. Contact support if you think this is a mistake.'
      : status === 'blocked'
        ? 'This account is blocked. Contact support for help.'
        : embedded
          ? "Staff will review your account. This page updates automatically when you're approved."
          : "Staff must approve your account. Stay on this page — you'll go to your feed when approved."
      ...
      <div className="flex flex-col gap-2 pt-1">
        {status === 'pending' ? (
          <button
            ...
            disabled={checking}
            className="w-full py-3 rounded-xl juwa2-btn text-sm font-semibold disabled:opacity-50 flex items-center justify-center gap-2"
          >
        ...
        <button
          ...
          className="w-full py-3 rounded-xl border border-white/15 bg-[#141414] hover:bg-[#151d39] flex items-center justify-center gap-2 text-sm font-semibold text-white"
        >
          <LogOut className="w-4 h-4" />
          Sign out
        </button>
        {!embedded ? (
          <Link href="/login" className="text-sm text-[#f5d040] hover:underline">
            Back to sign in
          </Link>
        ) : null}
      </div>
  ...
  return (
    <div className="min-h-screen bg-[#000000] text-white flex flex-col items-center justify-center p-6">
      <div className="max-w-md w-full rounded-3xl border border-white/10 bg-[#0c0c0c] p-8">{content}</div>
    </div>
  )
}
